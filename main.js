...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const speechBubble = document.getElementById('lolly-speech'); // speech bubble element
  const speechText = speechBubble ? speechBubble.querySelector('.speech-text') : null;
  if (!container) return;

  // Lolly's phrases for when she stops (one per stop)
  const lollyPhrases = [
  ...
  const pathStartLeft = 1200; // px where Lolly starts walking (updated)
  const pathStartTop = 280;    // px start top (updated)
  const pathMidTop = 400;     // px top at halfway (creates the U shape)
  ...
  const pathEndLeft = 340; // px where Lolly should disappear
  const pathEndTop = 205;  // px where Lolly should disappear
  ...
  function enterCabana() {
    ...
    // === STAND STILL FOR 1 SECOND BEFORE ENTERING ===
    const standStillMs = 1000; // how long Lolly stands still before moving into cabana
    
    // Get current position for standing still
    const currentLeft = gifWalk ? parseFloat(gifWalk.style.left) || 340 : 340;
    const currentTop = gifWalk ? parseFloat(gifWalk.style.top) || 205 : 205;
    
    // Show idle GIF, hide walk GIF immediately
    if (gifWalk) {
      gifWalk.style.cssText = `left:${currentLeft}px;top:${currentTop}px;opacity:0;display:none;position:absolute;`;
    }
    if (gifIdle) {
      gifIdle.style.cssText = `left:${currentLeft}px;top:${currentTop}px;opacity:1;display:block;position:absolute;z-index:2;`;
    }
    
    // After standing still, start the enter animation
    setTimeout(() => {
      ...
      const finishMs = 700;
      const el = useSprite && spriteLoaded ? spriteEl : (gifIdle || gifWalk);
      if (!el) return;
      ...
      el.style.transition = `left ${finishMs}ms linear, top ${finishMs}ms linear, opacity 0ms`;
      el.style.left = finalLeft + 'px';
      el.style.top = finalTop + 'px';
      // Ensure opacity is 1 at start of move
      el.style.opacity = '1';
      ...
      // Mark BOTH GIFs as fading out immediately to prevent step() from showing them
      if (gifWalk) gifWalk._lollyFadingOut = true;
      if (gifIdle) gifIdle._lollyFadingOut = true;
      if (spriteEl) spriteEl._lollyFadingOut = true;
      
      setTimeout(() => {
        el.style.transition = `left ${finishMs}ms linear, top ${finishMs}ms linear, opacity ${fadeDuration}ms linear`;
        el.style.opacity = '0';
        // Also hide the other GIF immediately
        if (gifWalk && gifWalk !== el) { gifWalk.style.display = 'none'; gifWalk.style.opacity = '0'; }
        if (gifIdle && gifIdle !== el) { gifIdle.style.display = 'none'; gifIdle.style.opacity = '0'; }
      }, Math.max(0, finishMs - fadeStartOffset));

      // hide a bit after fade completes
      setTimeout(() => {
        if (useSprite && spriteEl) { spriteEl.style.display = 'none'; spriteEl.style.opacity = '0'; }
        if (gifWalk) { gifWalk.style.display = 'none'; gifWalk.style.opacity = '0'; }
        if (gifIdle) { gifIdle.style.display = 'none'; gifIdle.style.opacity = '0'; }
  ...
  function step(now) {
    ...
    if (!onPath) {
      ...
      if (spriteEl) { spriteEl.style.left = pathStartLeft + 'px'; spriteEl.style.top = pathStartTop + 'px'; spriteEl.style.opacity = '1'; spriteEl.style.display = 'block'; spriteEl._lollyFadingOut = false; }
      if (gifWalk) { gifWalk.style.left = pathStartLeft + 'px'; gifWalk.style.top = pathStartTop + 'px'; gifWalk.style.opacity = '1'; gifWalk.style.display = 'block'; gifWalk._lollyFadingOut = false; }
      if (gifIdle) { gifIdle.style.left = pathStartLeft + 'px'; gifIdle.style.top = pathStartTop + 'px'; gifIdle.style.opacity = '1'; gifIdle.style.display = 'none'; gifIdle._lollyFadingOut = false; }
    }
    ...
    if (!pathPaused && t < 1) {
      for (let i = 0; i < pathStops.length; i++) {
        ...
        if (prevElapsed < stopMs && pathElapsedMs >= stopMs) {
          ...
          // Calculate current position for speech bubble
          const stopT = stop.t;
          const stopU = 1 - stopT;
          ...
          setTimeout(() => {
            hideSpeechBubble();
          }, stop.duration - 500); // hide slightly before resuming walk
    ...
    if (useSprite && spriteLoaded && spriteFrameWidth > 0) {
      ...
      if (spriteEl) {
        spriteEl.style.left = Math.round(bx) + 'px';
        spriteEl.style.top = Math.round(by) + 'px';
        // Prevent resetting opacity/display if fading out
        if (!spriteEl._lollyFadingOut) {
          spriteEl.style.opacity = '1';
          spriteEl.style.display = 'block';
        }
      }
    } else {
      // GIF fallback - ensure only ONE GIF is visible at a time
      if (gifWalk && gifIdle) {
        gifWalk.style.left = Math.round(bx) + 'px';
        gifWalk.style.top = Math.round(by) + 'px';
        gifIdle.style.left = Math.round(bx) + 'px';
        gifIdle.style.top = Math.round(by) + 'px';
        
        // Check if we're fading out
        const isFadingOut = gifWalk._lollyFadingOut || gifIdle._lollyFadingOut;
        
        if (isFadingOut) {
          // Hide both when fading out
          gifWalk.style.cssText = `left:${Math.round(bx)}px;top:${Math.round(by)}px;opacity:0;display:none;position:absolute;`;
          gifIdle.style.cssText = `left:${Math.round(bx)}px;top:${Math.round(by)}px;opacity:0;display:none;position:absolute;`;
        } else if (effectiveIdle) {
          // IDLE: show idle, hide walk
          gifWalk.style.cssText = `left:${Math.round(bx)}px;top:${Math.round(by)}px;opacity:0;display:none;position:absolute;`;
          gifIdle.style.cssText = `left:${Math.round(bx)}px;top:${Math.round(by)}px;opacity:1;display:block;position:absolute;z-index:2;`;
        } else {
          // WALKING: show walk, hide idle
          gifWalk.style.cssText = `left:${Math.round(bx)}px;top:${Math.round(by)}px;opacity:1;display:block;position:absolute;z-index:2;`;
          gifIdle.style.cssText = `left:${Math.round(bx)}px;top:${Math.round(by)}px;opacity:0;display:none;position:absolute;`;
        }
      }
    }
